 ...
 */
	public function view($id = null) {
		if (!$this->Estudio->exists($id)) {
			throw new NotFoundException(__('Estudio no válido'));
		}
	...
	public function add() {
		if ($this->request->is('post')) {
			...
			if ($this->Estudio->save($this->request->data)) {
				$this->Session->setFlash(__('El Estudio se ha guardado.'));
				return $this->redirect(array('action' => 'index'));
			} else {
				$this->Session->setFlash(__('El Estudio no se ha guardado. Por favor, inténte nuevamente.'));
			}
		}
	}
	...
	public function edit($id = null) {
		...
		if ($this->request->is(array('post', 'put'))) {
			if ($this->Estudio->save($this->request->data)) {
				...
				$this->Session->setFlash(__('El Estudio no se ha guardado. Por favor, verifique los datos.'));
			}
		} else {
			$options = array('conditions' => array('Estudio.' . $this->Estudio->primaryKey => $id));
 ...
 */
	public function delete($id = null) {
		...
		if ($this->Estudio->delete()) {
			$this->Session->setFlash(__('Tstudio has been deleted.'));
		} else {

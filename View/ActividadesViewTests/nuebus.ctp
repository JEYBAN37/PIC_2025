...
<div >
	...
	<?php
    echo $this->Form->create("ActividadesViewTest", array("action" => "nuebus"));
...
<table cellpadding="0" cellspacing="0" id="list" class="table table-bordered table-hover">
        <tr>
            ...
            if (empty($l)) {
                ...
                foreach ($l[0]["ActividadesViewTest"] as $i => $v) {
                    echo "<th>" . $i . "</th>";
                }
            }
            echo "<th>Ver</th>";
        ...
        <div class="actions">
            ...
            foreach ($l as $i => $v) {
                echo "<tr>";
                foreach ($v["ActividadesViewTest"] as $j => $w) {
                    echo "<td>" . $w . "</td>";
                }

                echo "<td>" . $this->Html->link("Ver", "../Actividades/view/" . $v["ActividadesViewTest"]["id"], array('target' => '_blank')) . "</td>";
                echo "<td> <a id='copi' class='copi' href='javascript:getlink(" . $v["ActividadesViewTest"]["id"] . ");' >Copiar URL </a></td>  " ;
                echo "<td>" . $this->Html->link("Editar", "../Actividades/edit/" . $v["ActividadesViewTest"]["id"]) . "</td>";
                echo "<td>" . $this->Html->link("Anexo", "../Actividades/editanexo/" . $v["ActividadesViewTest"]["id"]) . "</td>";
...
var location = locationAux.replace("actividadesviewtests/nuebus","Actividades/view/");
